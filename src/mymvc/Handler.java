...
public class Handler {
    ...
    //方法返回值 一个Object对象
    Object findObject(String className) throws ClassNotFoundException, IllegalAccessException, InstantiationException, NoSuchMethodException, InvocationTargetException {
        //1.上来先去集合里找对象
...
//    }
    ...
    private Object injectionDomain(Object obj,Class parameterClazz,HttpServletRequest request) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        ...
        for(Field field : fields){
            ...
            //=======================
            //==我自己加的  当实体对象有其他的属性时 应该跳过此次赋值过程
            //==当输入框不是String类型的时候 不输入是“” 不能转化为对应的空值
            if(value==null){
                continue;
            }
            ...
            //找寻属性类型对应的构造方法(基础的时候)
            if(value=="" && fieldType!=String.class){
                continue;
            }
        ...
        for(int i=0;i<parameters.length;i++){
            ...
            if(paramAnnotation!=null){//有注解  某一个基础类型 String int
                ...
                if(parameterClazz==Map.class || parameterClazz== List.class || parameterClazz==Set.class){
                    ...
                    Object obj = parameterClazz.getConstructor().newInstance();
